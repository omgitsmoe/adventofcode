...
// split charlist into a list of char pairs
let pList (p:char list) =
    p
    // unfold generates a seq as long as the generator function returns Some
    |> Seq.unfold (fun x ->
        match x with
        | a::b::tail -> Some ((a::b::[]), (b::tail))
        | _ -> None)
    |> List.ofSeq
...
// would be better to have paris as tuples rather than lists: ['a', 'b'] vs ('a', 'b')
let rulesMap =
    rules
    |> Map.fold (fun acc k v ->
        Map.add k ((List.head k::v::[]), (v::List.last k::[])) acc
        ) Map.empty
...
let applyRules3 (p:char list) times =
    ...
    let pairsMap =
        ...
        |> List.fold (fun acc pair -> Map.change pair (fun x ->
...
|> Map.fold (fun acc k count ->
    ...
    | _ -> failwith "Invalid pair") Map.empty
