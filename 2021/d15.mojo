...
# Pathfinding
# ------------------------------------------------------------
def find_path(mut grid: List[List[Node]], start: Pos, end_pos: Pos) -> Int:
    dimy = len(grid)
    dimx = len(grid[0])
    ...
    var visited: Set[Pos] = Set([start.copy()])
    ...
    while len(q) > 0:
        ...
        for npos in neighbours:
            ...
            ref neighbour_node = grid[npos.y][npos.x]
...
fn main():
    try:
        ...
        var grid: List[List[Node]] = [
            [Node(Int(c - zero_ascii), Int.MAX) for c in line.as_bytes()] for line in lines
        ]

        print("Part1:", find_path(grid, Pos(0, 0), Pos(len(grid) - 1, len(grid[0]) - 1)))

        # Part2: expand 5x
        var five_x_grid: List[List[Node]] = []
        for yy in range(5):
            for line in lines:
                var row: List[Node] = []
                for xx in range(5):
                    for c in line.as_bytes():
                        ...
                        row.append(Node(new_level, Int.MAX))
                five_x_grid.append(row^)

        start = time.perf_counter()
        print("Part2:", find_path(five_x_grid, Pos(0, 0), Pos(len(five_x_grid) - 1, len(five_x_grid[0]) - 1)))
